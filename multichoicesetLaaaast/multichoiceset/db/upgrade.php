...
function xmldb_qtype_multichoiceset_upgrade($oldversion) {
    ...
    // added version
    if ($oldversion < 2019040140) {
		$table = new xmldb_table('qtype_multichoiceset_options');
        $field = new xmldb_field('num_of_wrong_answers', XMLDB_TYPE_INTEGER, '2',
                null, XMLDB_NOTNULL, null, '0', null);
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        
        $field = new xmldb_field('minus_marks', XMLDB_TYPE_INTEGER, '2',
                null, XMLDB_NOTNULL, null, '0',null);
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }
        upgrade_plugin_savepoint(true, 2019040140, 'qtype', null);
    }
